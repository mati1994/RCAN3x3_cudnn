...
// TODO(mt): added by mt
inline void readOpenCVFile(const std::string& fileName, uint8_t* buffer, int ch, int inH, int inW)
{
	cv::Mat src = cv::imread(fileName, cv::IMREAD_UNCHANGED);
	//cv::Mat src_resize(inH, inW, CV_8UC1);
	//cv::resize(src, src_resize, src_resize.size() );
	int type;
	if(ch == 1)
		type = CV_8UC1;
	else
		...
		type = CV_8UC3;
	cv::Mat dst(inH, inW, type, static_cast<void*>(buffer) );
	std::cout << "DEBUG, buffer address is " << static_cast<void*>(buffer) 
		<< " while dst.data address is " << static_cast<void*>(dst.data) << std::endl;
	cv::resize(src, dst, dst.size() );
	//src_resize.copyTo(dst);
	std::cout << "DEBUG, after src.copyTo, buffer address is " << static_cast<void*>(buffer) 
		<< " while dst.data address is " << static_cast<void*>(dst.data) << std::endl;
}
